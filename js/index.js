$(function () {
  var options = {
    ...
    useEasing: true, // 是否缓动动画
    smartEasingThreshold: 999, // 如果设置了缓动, 则对大于下值的数缓动
    smartEasingAmount: 333, // 超过阈值
    separator: ',', // 分隔符
    decimal: '.', // 小数点符号
...
function countNum(el, num, opt) {
  const count = new CountUp(el, num, opt)
  if (!count.error) {
    count.start();
  } else {
...
function serviceHover() {
  $('#service .item-box').mouseenter(function () {
    $(this).addClass('active').parent().siblings().children('.item-box').removeClass('active')
  })
}
